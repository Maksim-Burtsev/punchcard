...
concurrent request fail and the same request twice a no-op, which one guard every thread takes to
reach shared state in memory, and whether the new stored or transmitted shape survives old and new
code running at the same time. State that left the system's control and re-enters the process — a hidden
...
**Sources:** (02, 04, 05, 16, 20, 21, 27, 32; F3)
...
**Sources:** (15, 16, 20, 21, 30, 32, 34; F9, F10)

### 5.3 Make the store refuse the second concurrent writer and the second delivery, and never let a clock decide
...
paused process writes as if still in charge, so only the resource itself can refuse it. Where the
write is reachable by a redelivery, a retry or a replay, a second question is owed beside the first:
what makes the same request twice a no-op? A uniqueness constraint on a business tuple answers
neither question and is a finding under both — it collides the day the business legitimately repeats
that key, and it lets a genuine duplicate through whenever any field in the tuple differs. What makes
a constraint answer the duplication question is an identifier the sender minted for this attempt — a
message id, a client request id — carried end to end and stored with the effect. Where the operation
admits absolute-value semantics, set the balance rather than add to it and the question does not
arise. A seen-id history is the fallback, not the default, and it owes this principle back to itself:
the seen-id write and the effect commit together or the history lies about work that never happened,
and it ships with a bound and an eviction path.
...
separate lock service adds nothing. A write no redelivery, retry or replay path can reach owes only
the first question, and a naturally idempotent one — a query, an absolute-value set — owes neither;
some operations are legitimately non-idempotent, where each request is meant to create a new
instance. Whether the retry that produces the duplicate is itself bounded, backed off and jittered
is 6.6's, not this principle's.
**Sources:** (15, 16, 27, 30, 31, 32, 34; F10, F18)
...
separate shape.
**Sources:** (15, 16, 20, 21, 30, 32, 34; F4, F12)
